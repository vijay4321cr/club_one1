...
| View online tickets (logged-in) | `{DOMAIN}/account` | `/account` | Shows ALL of the user's tickets with QR codes (via `/order/userTickets` + `/order/viewTicketsWithTicketId`). Requires phone-OTP login. |
| Deep-link one order's QR (logged-in) | `{DOMAIN}/account?order=on_7a74...` | `/account?order={orderId}` | Login-gated in-app deep link — opens that order's QR modal (tickets or table). **Not** for email (needs login). Set automatically when a user taps View QR. |
| View ticket as guest (login-free SMS/email link) | `{DOMAIN}/ticket/view?id=6a5f...&token=<64hex>` | `/ticket/view?id={orderId}&token={guestToken}` | ✅ Implemented. Calls `GET /order/viewTicketsGuest/{orderId}?token={token}` — no login. Shows tickets + entry QRs immediately. This is the preferred target for booking SMS/email. |
