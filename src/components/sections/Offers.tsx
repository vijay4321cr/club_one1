      ...
      <div className="grid gap-px overflow-hidden rounded-sm bg-line md:grid-cols-2">
        {offers.map((o, i) => (
          <FxReveal key={o.id} effect="tilt" delay={(i % 2) * 0.12}>
            {/* tapping an offer → contact page to enquire / claim */}
            <TransitionLink
              href={`/contact?offer=${encodeURIComponent(o.title)}`}
              className="group flex h-full flex-col justify-between gap-8 bg-coal p-6 transition-colors duration-500 hover:bg-surface md:p-10"
            >
              ...
              <div>
                ...
                <span className="label mt-5 inline-flex items-center gap-2 !text-primary">
                  Know more
